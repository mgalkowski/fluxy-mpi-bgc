import inspect
from fluxy.io import extract_site_info
import numpy as np
            ...
            for p in plot_point_markers:
                if type(p) == list:
                    ...
                    if p not in config.point_source_dict.keys():
                        ...
                        ax0.scatter(config.point_source_dict[p][0],config.point_source_dict[p][1], facecolors='none', 
                                    edgecolors='red',marker='^',s=30,zorder=2)
                        ax1.scatter(config.point_source_dict[p][0],config.point_source_dict[p][1], facecolors='none', 
                                    edgecolors='red',marker='^',s=30,zorder=2)
                        ax2.scatter(config.point_source_dict[p][0],config.point_source_dict[p][1], facecolors='none', 
